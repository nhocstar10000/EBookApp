...
import static com.example.e_bookapp.Constants.MAX_BYTES_PDF;
...
public class MyAppication extends Application {
    ...
    public static void deletedBook(Context context,String bookId, String bookUrl, String bookTitle) {
        String TAG = "DELETE_BOOK_TAG";

        Log.d(TAG, "deletedBook: Deleting");
        ProgressDialog progressDialog = new ProgressDialog(context);
        progressDialog.setTitle("Please wait");
        progressDialog.setMessage("Deleting" + bookTitle);
        progressDialog.show();

        Log.d(TAG, "deletedBook: Deleting from storage");
    ...
    public static void incrementBooksViewCount(String bookId){
    ...
    private static void saveDownloadedBook(Context context, ProgressDialog progressDialog, byte[] bytes, String nameWithExtension, String bookId) {
        ...
        try{
            ...
            progressDialog.dismiss();
            incrementBooksDownloadCount(bookId);
        }
        catch (Exception e){
    ...
    private static void incrementBooksDownloadCount(String bookId) {
        Log.d(TAG_DOWNLOAD, "incrementBooksDownloadCount: Incrementing Book Download Count");
        DatabaseReference ref = FirebaseDatabase.getInstance().getReference("Books");
        ref.child(bookId)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
        ...
        if (firebaseAuth.getCurrentUser() == null){
            ...
            ref.child(firebaseAuth.getUid()).child("Favorite").child(bookId)
                    ...
                    .addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(context, "Failed to add your favorites due to" + e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
        }

    }
    public static void removetoFavorite(Context context, String bookId){
        FirebaseAuth firebaseAuth = FirebaseAuth.getInstance();
        if (firebaseAuth.getCurrentUser() == null){
            Toast.makeText(context, "You are not logged in", Toast.LENGTH_SHORT).show();
        }
            ...
            ref.child(firebaseAuth.getUid()).child("Favorite").child(bookId)
                    ...
                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void unused) {
                            Toast.makeText(context, "Remove from your favorites", Toast.LENGTH_SHORT).show();
                        }
                    })
                    .addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(context, "Failed to  remove your favorites due to" + e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
        }
        ...
        if (firebaseAuth.getCurrentUser() == null) {
            ...
            ref.child(firebaseAuth.getUid()).child(bookId)
                    ...
                    .addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            Toast.makeText(context, "Failed to add your cart due to" + e.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
        }
    }
